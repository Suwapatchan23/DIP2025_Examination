...
if __name__ == "__main__":
    ...
    input_path = r"../Datasets/Mushroom_Segmentation_2569/images/mushroom_1.png"
    ...
    # -> Color Range Segmentation (Euclidean)
    # eu_seg_img = colorRange(rgb_img, [160, 160, 160], 150, "Euclidean")
    # eu_seg_img = eu_seg_img.astype(np.uint8)

    # lower_bound = (17, 9, 79)
    # upper_bound = (116, 190, 240)
    # ### => color range segmentation
    # output_img = cv.inRange(hsv_img, lower_bound, upper_bound)

    lower_stipe = np.array([0, 0, 58])
    upper_stipe = np.array([255, 255, 255])
    ...
    output_mask = getLargestConnectedComponent(fillhole_img).astype(np.uint8)
